...
def minimize_phi_row(phi_prev, psi_scalars, observed_phi_row, sigma_row, alphas, ns, m, row_idx=None, parallelized=False, shared_array_id=None):
    ...
    try:
        ...
        prob = cp.Problem(cp.Minimize((observed_phi_row - phi_next)**2 @ sample_coef.T +
                                      (phi_next @ alphas.T - psi_scalars / ns)**2 @ bulk_coef.T), [phi_next >= 0])
        prob.solve()
...
def minimize_alpha(alpha_prev, sigma, phi, psi, n):
    ...
    try:
        ...
        prob = cp.Problem(cp.Minimize(
            coef.T @ (phi @ alpha_next.T - psi / n)**2, [alpha_next >= 0, cp.sum(alpha_next) == 1]))
        prob.solve()
...
def alternate_minimization(phi, sigma, m, psis, alpha_inits, n_inits, eps, delta, max_iter, parallelized=False, num_process=1):
    L_past = compute_full_likelihood(
        phi, phi, psis, alpha_inits, sigma, n_inits, m)
    phi_next = minimize_phi(phi, psis, phi, sigma,
                            alpha_inits, n_inits, m, parallelized, num_process)
    alpha_nexts = np.zeros(alpha_inits.shape)
    for i in range(alpha_nexts.shape[0]):
        alpha_nexts[i] = minimize_alpha_LS_helper(alpha_inits[i].reshape(
            1, -1), sigma, phi_next, psis[:, i].reshape(-1, 1), n_inits[i], ALPHA_EPS, ALPHA_MAX_ITER)
...
def minimize_phi_row_grad(phi_past, observed_phi_row, alphas, ns, observed_psi_scalars, sigma_row, m, row_idx=None, parallelized=False, shared_array_id=None):
    ...
    res = scipy.optimize.minimize(lambda phi_row: compute_row_likelihood(phi_row.reshape(1, -1), observed_phi_row,

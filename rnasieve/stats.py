...
def cross_protocol_inverse_observed_fisher(phi, sigma, m, alpha, psi, n):
    ...
    z_tail = np.clip(np.count_nonzero((z_scores > np.median(z_scores) + 2) | (z_scores < np.median(z_scores) - 2)), CLIP_VALUE, None)
    filter_length = np.min((np.round(3 * z_scores.shape[0] / z_tail), 300))
    filter_idxs = np.where(z_scores > np.quantile(z_scores, (z_scores.shape[0] - filter_length) / z_scores.shape[0]))[0]

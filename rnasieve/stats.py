...
def _partial_phi_phi(phi, sigma, alpha, n, m):
    G, K = phi.shape
    mixture_variance = compute_mixture_sigma(alpha, sigma, phi)
    alpha_term = np.repeat(n * np.kron(alpha, alpha).reshape(K, K)
                           [:, :, np.newaxis], G, axis=2) / mixture_variance.reshape(1, 1, -1)
...
def inverse_observed_fisher(phi, sigma, alpha, n, m):
    ...
    return ((inv_fisher + inv_fisher.T) / 2)[:alpha.shape[1] + 1, :alpha.shape[1] + 1]
...
def cross_protocol_inverse_observed_fisher(phi, sigma, m, alpha, psi, n):
    ...
    z_tail = np.clip(np.count_nonzero((z_scores > np.median(
        z_scores) + 2) | (z_scores < np.median(z_scores) - 2)), CLIP_VALUE, None)
    ...
    return inverse_observed_fisher(phi[filter_idxs], np.clip(sigma[filter_idxs], 1, None), alpha, n, m)
